...
export default function CallOverlay({ currentUserId, selectedUser }: { currentUserId: string; selectedUser: Profile | null }) {
  ...
  useEffect(() => {
    const handler = (event: Event) => {
      const kind = (event as CustomEvent<CallKind>).detail;
      if (kind === "audio" || kind === "video") void startCall(kind);
    };
    window.addEventListener("banda:start-call", handler);
    return () => window.removeEventListener("banda:start-call", handler);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selectedUser, state, currentUserId]);
      ...
      {state !== "idle" && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/80 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md overflow-hidden rounded-3xl bg-white shadow-2xl">
            ...
            {kind === "video" && state !== "incoming" && <div className="relative aspect-video bg-black"><video ref={remoteVideoRef} autoPlay playsInline className="h-full w-full object-contain" /><video ref={localVideoRef} autoPlay muted playsInline className="absolute bottom-3 right-3 h-24 w-32 rounded-xl bg-slate-800 object-cover shadow-lg" /></div>}
