...
export default function GroupsPage() {
    ...
    (async () => {
      ...
      if (mounted) {
        ...
        if (requestedGroupId) {
          ...
          if (requestedGroup) {
            setSelectedGroup(requestedGroup as Group);
            await markGroupRead(requestedGroup.id);
          }
        }
        setLoading(false);
      }
  ...
  async function markGroupRead(groupId: string) {
    const { error: readError } = await supabase.rpc("mark_banda_group_read", { p_conversation_id: groupId });
    if (readError) console.error("Mark group read error:", readError);
  }
  ...
  function openEditGroup() {
    ...
    setEditAvatarPreview(selectedGroup.group_avatar_url || null);
    setError("");
    setShowEdit(true);
  }
  ...
  async function uploadGroupAvatar(file: File, groupId: string) {
    ...
    if (!data.publicUrl) throw new Error("URL foto grup tidak ditemukan.");
  ...
  async function saveGroupEdit() {
    ...
    try {
      let avatarUrl = selectedGroup.group_avatar_url || null;
      if (editAvatarFile) avatarUrl = await uploadGroupAvatar(editAvatarFile, selectedGroup.id);
      ...
      setError(err instanceof Error ? err.message : "Grup gagal diperbarui.");
